...
\geometry{verbose,letterpaper,tmargin=1.5cm,bmargin=2cm,lmargin=1.6cm,rmargin=1.6cm}
...
\section{Teoría de números}
  ...
  Encuentra los primos desde 1 hasta un límite $n$. \verb|sieve[i]| es falso sí y solo sí i es un número primo.\\
  Complejidad: muy próxima a $O(n)$ donde $n$ es el límite superior, pero en realidad la complejidad es $O(n loglogn)$.\\
  \source{./src/sieve.cpp}\\
  Version optimizada  \source{./src/sieve_optimized.cpp}\\
  Version en python: como python trabaja sobre referencias, y la idea es no tener que lidiar mucho que la palabra $global$, lo ideal es retornar la referencia, ademas $MAXN$ se puede parámetrizar (pasar por argumento) si se desea
  \source{./src/sieve.py}
